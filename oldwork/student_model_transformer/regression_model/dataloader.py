...
class SequenceLoader(object):
    ...
    def __init__(self, data_folder, source_suffix, target_suffix, split, tokens_in_batch,bleu_scores=None):
        """
        ...
        assert split.lower() in {"train", "val",
                                 "test","student_train"}, "'split' must be one of 'train', 'val', 'test'! (case-insensitive)"
        self.split = split.lower()

        # Is this for training?
        self.for_training = True if "train" in self.split else False

        # Load BPE model
        self.bpe_model = youtokentome.BPE(model=os.path.join(data_folder, "bpe_withs.model"))
        ...
        # Concat BLEU score for Calculate the regression Loss
        if bleu_scores is not None:
            # print(len(bleu_scores))
            # print(len(target_data))
            assert len(bleu_scores) == len(target_data), "The number of BLEU scores must match the number of target sequences!"
            self.data = list(zip(source_data, target_data, source_lengths, target_lengths,bleu_scores))
        else:
    ...
    def __next__(self):
        ...
        self.current_batch += 1
        try:
            # Check if there is BLEU score in the data
            if len(self.data[0]) == 5:
                source_data, target_data, source_lengths, target_lengths, bleu_scores = zip(*self.all_batches[self.current_batch])
        ...
        return source_data, target_data, source_lengths, target_lengths,bleu_scores
