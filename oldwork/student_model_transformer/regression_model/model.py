...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_sequences, decoder_sequence_lengths, encoder_sequences, encoder_sequence_lengths):
        ...
        # Generate Regression Token for each hiddenstate

        avg_pool = torch.mean(decoder_sequences, dim=1) 

        regression_score = self.probability_layer(avg_pool).squeeze(-1)

        # Find logits over vocabulary
        decoder_sequences = self.fc(decoder_sequences)  # (N, pad_length, vocab_size)

        return decoder_sequences,regression_score
...
class Transformer(nn.Module):
    ...
    def forward(self, encoder_sequences, decoder_sequences, encoder_sequence_lengths, decoder_sequence_lengths):
        ...
        # Decoder
        decoder_sequences,regression_score = self.decoder(decoder_sequences, decoder_sequence_lengths, encoder_sequences,
...
class TransformerWithProbability(nn.Module):
    def __init__(self, transformer_model):
        super(TransformerWithProbability, self).__init__()
        self.transformer_model = transformer_model
        self.probability_layer = nn.Sequential(
            nn.Linear(transformer_model.config.hidden_size, 1),  # 降维到1维
            nn.Sigmoid()  # Sigmoid激活函数
        )

    def forward(self, input_ids, decoder_input_ids):
        # 获取Transformer模型的输出
        outputs = self.transformer_model(input_ids=input_ids, decoder_input_ids=decoder_input_ids)
        last_hidden_states = outputs.last_hidden_state  # 获取解码器的最后一个隐状态

        # 应用额外的全连接层和Sigmoid激活函数
        probabilities = self.probability_layer(last_hidden_states).squeeze(-1)
        return probabilities

# 假设transformer_model是你的Transformer模型实例
# transformer_model = ...

# 创建包含额外概率层的模型实例
# model_with_prob = TransformerWithProbability(transformer_model)
